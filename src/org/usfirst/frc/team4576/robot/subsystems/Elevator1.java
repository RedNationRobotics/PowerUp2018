...
public class Elevator1 extends Subsystem {
	...
	public WPI_TalonSRX tsrxE = new WPI_TalonSRX(RobotMap.ELEVATOR_TALON); /*
										 * Inversed numbers to compensate for
										 * difference from practice bot
										 */
	...
	public Elevator1() {
		...
		tsrxE.config_kP(RobotMap.kPIDLoopIdx, 0.1, RobotMap.kTimeoutMs);
		tsrxE.config_kI(RobotMap.kPIDLoopIdx, 0.0, RobotMap.kTimeoutMs);
		tsrxE.config_kD(RobotMap.kPIDLoopIdx, 0.0, RobotMap.kTimeoutMs);
		...
		tsrxE.setSelectedSensorPosition(absolutePosition, RobotMap.kPIDLoopIdx, RobotMap.kTimeoutMs);
	...
	public void elevatorTeleop(Joystick stick)
	{
		if(firstRun)
		{
			targetPos1 += tsrxE.getSelectedSensorPosition(RobotMap.kPIDLoopIdx);
			targetPos2 += tsrxE.getSelectedSensorPosition(RobotMap.kPIDLoopIdx);
			firstRun = !firstRun;
		}
		/* get gamepad axis */
		...
		//elevstring.append("45 is set to: " + targetPos1 + " which should be .125 rotations from intial\n");
		elevstring.append("\tout:");
		elevstring.append(motorOutput);
		elevstring.append("\tpos:");
		elevstring.append(tsrxE.getSelectedSensorPosition(RobotMap.kPIDLoopIdx));
		/* on button1 press enter closed-loop mode on target position */
		if (!_lastButton1 && button1) {
			
			tsrxE.set(ControlMode.Position, targetPos1); /*50 rotations in either direction*/

		}
		if (!_lastButton2 && button2) {
			
			tsrxE.set(ControlMode.Position, targetPos2); /*50 rotations in either direction*/

		}
		//might be needed
		
//		if(tsrxE.getControlMode() == ControlMode.PercentOutput) tsrxE.set(leftYstick);
